...
self.addEventListener('fetch',event=>{
  ...
  if(isPage){
    event.respondWith(fetch(event.request).then(async r=>{
      ...
      patched=patched.replace(/gold-v1\.js\?v=\d+/g,'gold-v1.js?v=3');
      if(!patched.includes('gold-v1.js'))patched=patched.replace('</body>','<script src="./gold-v1.js?v=3"></script></body>');
