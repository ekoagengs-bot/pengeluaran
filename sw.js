const CACHE='monikas-v39-gold-native';
const APP_ASSETS=['./','./index.html','./manifest.json','./icon.svg','./gold-v1.js'];
...
self.addEventListener('fetch',event=>{
 ...
 if(isPage){event.respondWith(fetch(event.request).then(async r=>{
   ...
   let patched=text.replace(/<script[^>]*gold-force-v4\.js[^>]*><\/script>/gi,'').replace(/<script[^>]*gold-v1\.js[^>]*><\/script>/gi,'');
   patched=patched.replace('</body>','<script src="./gold-v1.js?v=5"></script></body>');
