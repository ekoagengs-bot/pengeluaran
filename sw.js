const CACHE='monikas-v28-dashboard-fix';
const APP_ASSETS=['./','./index.html','./manifest.json','./icon.svg','./dashboard-fix-v28.js'];
...
self.addEventListener('fetch',event=>{
  ...
  if(isPage){
    event.respondWith(fetch(event.request).then(async r=>{
      ...
      const injected=text.replace('</body>','<script src="./dashboard-fix-v28.js?v=28"></script></body>');
      ...
      const text=await r.text();
      return new Response(text.replace('</body>','<script src="./dashboard-fix-v28.js?v=28"></script></body>'),{headers:{'Content-Type':'text/html; charset=utf-8'}});
    }));
    return;
  }
