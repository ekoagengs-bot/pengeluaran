const CACHE='monikas-v15-funds-fixed-1';
...
self.addEventListener('fetch',event=>{
 ...
 if(isPage){
  event.respondWith(fetch(event.request).then(async response=>{
   ...
   if(!injected.includes('monitoring.js'))injected=injected.replace('</body>','<script src="./monitoring.js?v=15"></script></body>');
   injected=injected.replace('</body>','<script src="./funds-final.js?v=15"></script></body>');
