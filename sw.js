const CACHE='monikas-v29-dashboard';
const APP_ASSETS=['./','./index.html','./manifest.json','./icon.svg','./dashboard-fix-v29.js'];
...
self.addEventListener('fetch',event=>{
  ...
  if(isPage){
    event.respondWith(fetch(event.request).then(async r=>{
      ...
      const patched=text.includes('dashboard-fix-v29.js')?text:text.replace('</body>','<script src="./dashboard-fix-v29.js?v=29"></script></body>');
      ...
      return new Response(text.replace('</body>','<script src="./dashboard-fix-v29.js?v=29"></script></body>'),{headers:{'Content-Type':'text/html; charset=utf-8'}});
    }));
    return;
  }
