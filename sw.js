...
self.addEventListener('fetch',event=>{
  ...
  if(isPage){
    event.respondWith(fetch(event.request).then(async r=>{
      ...
      if(!patched.includes('gold-v1.js'))patched=patched.replace('</body>','<script src="./gold-v1.js?v=1"></script></body>');
      return new Response(patched,{status:r.status,statusText:r.statusText,headers:r.headers});
    }).catch(()=>caches.match('./index.html').then(async r=>{if(!r)return new Response('MoniKas offline',{status:503});const text=await r.text();let patched=text.replace('</body>','<script src="./pos-sync-fix-v33.js?v=33"></script></body>');patched=patched.replace('</body>','<script src="./gold-v1.js?v=1"></script></body>');return new Response(patched,{headers:{'Content-Type':'text/html; charset=utf-8'}});}));
    return;
  }
