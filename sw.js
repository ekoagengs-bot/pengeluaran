const CACHE='monikas-v33-pos-dashboard';
const APP_ASSETS=['./','./index.html','./manifest.json','./icon.svg','./pos-sync-fix-v33.js'];
...
self.addEventListener('fetch',event=>{
  ...
  if(isPage){
    event.respondWith(fetch(event.request).then(async r=>{
      ...
      const patched=text.includes('pos-sync-fix-v33.js')?text:text.replace('</body>','<script src="./pos-sync-fix-v33.js?v=33"></script></body>');
      ...
      return new Response(text.replace('</body>','<script src="./pos-sync-fix-v33.js?v=33"></script></body>'),{headers:{'Content-Type':'text/html; charset=utf-8'}});
    }));
    return;
  }
