const CACHE='monikas-v20-form-stable-1';
const APP_ASSETS=['./','./index.html','./manifest.json','./icon.svg','./backend-config.js','./sheet-sync.js','./monitoring.js','./form-bridge-v2.js'];
...
self.addEventListener('fetch',event=>{
 ...
 if(isPage){
  event.respondWith(fetch(event.request).then(async response=>{
   ...
   html=html.replace(/<script[^>]+pos-dana\\.js[^>]*><\\/script>/gi,'')
     .replace(/<script[^>]+funds-v5\\.js[^>]*><\\/script>/gi,'')
     .replace(/<script[^>]+funds-final\\.js[^>]*><\\/script>/gi,'')
     .replace(/<script[^>]+monitoring\\.js[^>]*><\\/script>/gi,'')
     .replace(/<script[^>]+form-restore-v1\\.js[^>]*><\\/script>/gi,'')
     .replace(/<script[^>]+form-bridge-v2\\.js[^>]*><\\/script>/gi,'');
   html=html.replace('</body>','<script src="./monitoring.js?v=20"></script><script src="./form-bridge-v2.js?v=20"></script></body>');
